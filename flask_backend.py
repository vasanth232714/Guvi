"""
Hospital Analytics Dashboard - Flask Backend API (Railway Deployment Version)
...
# Enable CORS for all domains (you can restrict this in production)
CORS(app, resources={
    r"/api/*": {
        "origins": "*",
...
DB_CONFIG = {
    ...
    'database': os.getenv('MYSQLDATABASE', 'hospital_analytics'),
...
def get_db_connection():
    """Create and return database connection with retry logic"""
    max_retries = 3
    retry_delay = 2
...
def index():
    """Root endpoint"""
    return jsonify({
        'message': 'Hospital Analytics API',
        'version': '2.0',
        'status': 'running',
        'endpoints': [
            '/api/health',
            ...
            '/api/branches/comparison',
            '/api/doctor-utilization',
